...
function Wiki(props) {
  ...
  {
    userService.sendGETRequest('/questions/wiki', { 
      uuid: uuid,
      network_id: values.network_id
    },
    function(result){
      setValues({ ...values, 
        name: result[0]['wiki']['name'],
        description: result[0]['wiki']['description']
      });
  ...
  function submit(){
    ...
    userService.sendRequest('/questions/wiki/', option, {
        uuid: values.uuid,
        network_id: values.network_id,
        name: values.name,
    ...
    function(result){
        ...
        const msg = values.uuid.length === 0 ? "Wiki cadastrado!" : "Wiki atualizado!";
