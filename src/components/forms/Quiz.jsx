...
function Quiz(props) {
  ...
  {
    userService.sendGETRequest('/questions/quiz', { 
      uuid: uuid,
      network_id: values.network_id
    },
    function(result){
      setValues({ ...values, 
        name: result[0]['quiz']['name'],
        description: result[0]['quiz']['description']
      });
  ...
  function submit(){
    ...
    userService.sendRequest('/questions/quiz/', option, {
        uuid: values.uuid,
        network_id: values.network_id,
        name: values.name,
    ...
    function(result){
        ...
        const msg = values.uuid.length === 0 ? "Quiz cadastrado!" : "Quiz atualizado!";
