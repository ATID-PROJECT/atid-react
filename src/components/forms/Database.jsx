  ...
  const carregarDados = function(uuid) {
    userService.sendGETRequest(
      ...
      function(result) {
        setValues({
          ...values,
          name: result[0]["database"]["name"],
          description: result[0]["database"]["description"]
        });
      }
    );
  };
  ...
  function submit() {
    ...
    userService.sendRequest(
      ...
      function(result) {
        ...
        const msg =
          values.uuid.length === 0
            ? "atividade 'Base de dados' cadastrada!"
  ...
  return (
    <div className={classes.root}>
      <ExpansionPanel
        ...
        onChange={() => setValues({ ...values, expanded: !values.expanded })}
      >
      ...
      <ExpansionPanel>
        <ExpansionPanelSummary expandIcon={<ExpandMoreIcon />}>
          <Typography className={classes.heading}>Disponibilidade</Typography>
        </ExpansionPanelSummary>
        <ExpansionPanelDetails>
