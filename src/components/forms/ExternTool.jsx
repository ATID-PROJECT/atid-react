...
function ExternTool(props) {
  ...
  {
    userService.sendGETRequest('/questions/externtool', { 
      uuid: uuid,
      network_id: values.network_id
    },
    function(result){
      setValues({ ...values, 
        name: result[0]['externtool']['name'],
        description: result[0]['externtool']['description']
      });
  ...
  function submit(){
    ...
    userService.sendRequest('/questions/externtool/', option, {
        uuid: values.uuid,
        network_id: values.network_id,
        name: values.name,
    ...
    function(result){
        ...
        const msg = values.uuid.length === 0 ? "Atividade cadastrada!" : "Atividade atualizada!";
...
ExternTool.propTypes = {
  id_network: PropTypes.string,
};

export default ExternTool;
