...
function Glossario(props) {
  ...
  {
    userService.sendGETRequest('/questions/glossario', { 
      uuid: uuid,
      network_id: values.network_id
    },
    function(result){
      setValues({ ...values, 
        name: result[0]['glossario']['name'],
        description: result[0]['glossario']['description']
      });
  ...
  function submit(){
    ...
    userService.sendRequest('/questions/glossario/', option, {
        uuid: values.uuid,
        network_id: values.network_id,
        name: values.name,
    ...
    function(result){
        ...
        const msg = values.uuid.length === 0 ? "Glossario cadastrado!" : "Glossario atualizado!";
